"""
Main Orchestrator for Teradata Lineage Analysis with LLM-based Procedure Analysis
Coordinates cleaning, analysis, and JSON building for complete database lineage

This module analyzes SQL files from a directory (file-based analysis)
Teradata-specific features supported:
- MACRO definitions
- Stored Procedures (REPLACE PROCEDURE)
- User-Defined Functions (UDF)
- Triggers
- Multi-statement requests (semicolon-separated)
- BTEQ commands and scripts
- FastLoad, MultiLoad, TPump scripts
...
class LineageOrchestrator:
    ...
    def run_full_analysis(self, max_files: int = None) -> Dict:
        ...
        print("\n" + "="*80)
        print("TERADATA LINEAGE ANALYSIS PIPELINE (with LLM-Enhanced Procedure Analysis)")
        print("="*80)
        ...
        if self.macro_results:
            print(f"   ✓ Found {len(self.macro_results)} macros")

        # Step 3: Extract procedures, functions, macros, views, and triggers for LLM analysis
        print("\n[3/6] Extracting procedures, functions, macros, views, and triggers...")
        ...
        print(f"   ✓ Found {len([s for s in proc_func_statements if s['type'] in ['FUNCTION', 'UDF']])} functions")
        macro_count = len([s for s in proc_func_statements if s['type'] == 'MACRO'])
        if macro_count > 0:
            print(f"   ✓ Found {macro_count} macros")
        view_count = len([s for s in proc_func_statements if s['type'] == 'VIEW'])
        if view_count > 0:
            print(f"   ✓ Found {view_count} views")
        trigger_count = len([s for s in proc_func_statements if s['type'] == 'TRIGGER'])
        if trigger_count > 0:
            print(f"   ✓ Found {trigger_count} triggers")

        # Step 4: Perform LLM-based analysis on procedures/functions/macros/views/triggers
        print("\n[4/6] Analyzing procedures/functions/macros/views/triggers with LLM...")
        ...
        if proc_func_statements:
            ...
            print("   ⚠ No procedures, functions, macros, views, or triggers found to analyze")
        ...
        print("\n[6/6] Building LLM-enhanced procedure/function/view lineage report...")

        # Add LLM results to procedure JSON builder
        all_llm_results = self.llm_analyzed_procedures + self.llm_analyzed_functions + self.llm_analyzed_macros + self.llm_analyzed_views + self.trigger_results
        ...
        return {
            ...
            'statistics': {
                ...
                'total_views': len(self.llm_analyzed_views),
        ...
        for file_path, content in self.cleaned_files.items():
            ...
            if content.get('views'):
                for view in content['views']:
                    ...
                    # Also add view to statement_results so it appears in statement lineage JSON
                    self.statement_results.append(view_result)

            # Track macros (Teradata-specific)
            if content.get('macros'):
                for macro in content['macros']:
                    self.macro_results.append({
                        'name': macro['name'],
                        'file_path': file_path,
    ...
    def _extract_procedures_and_functions(self) -> List[Dict]:
        """
        Extract procedures, functions, macros, views, and triggers from cleaned files and prepare them for LLM analysis.
        ...
        for file_path, content in self.cleaned_files.items():
            ...
            if content.get('functions'):
                for func in content['functions']:
                    ...
                    statement = {
                        'type': func_type,  # Could be FUNCTION, UDF
                        'name': func_name,
            ...
            # Extract macros (Teradata-specific - parameterized SQL)
            if content.get('macros'):
                for macro in content['macros']:
                    ...
                    statement = {
                        'type': 'MACRO',
                        'name': macro_name,
                        'original_sql': macro_content,
                        'modified_sql': macro_content,
                        'file_path': str(file_path),
                        'schema_name': self._extract_schema_from_name(macro_name),
                        'object_name': self._extract_object_name(macro_name)
                    }
                    proc_func_statements.append(statement)

            # Extract views for LLM analysis
            if content.get('views'):
                for view in content['views']:
                    ...
                    statement = {
                        'type': 'VIEW',
                        'name': view_name,
                        'original_sql': view_content,
                        'modified_sql': view_content,
                        'file_path': str(file_path),
                        'schema_name': self._extract_schema_from_name(view_name),
                        'object_name': self._extract_object_name(view_name)
                    }
                    proc_func_statements.append(statement)
    ...
    def _extract_schema_from_name(self, full_name: str) -> str:
        """Extract schema/database from qualified name (e.g., 'DATABASE.ProcName' -> 'DATABASE')"""
        if '.' in full_name:
            return full_name.split('.')[0].strip('"')
        return 'DBC'  # Default Teradata system database

    def _extract_object_name(self, full_name: str) -> str:
        """Extract object name from qualified name (e.g., 'DATABASE.ProcName' -> 'ProcName')"""
        if '.' in full_name:
    ...
    def _calculate_llm_success_rate(self) -> str:
        ...
        total = len(self.llm_analyzed_procedures) + len(self.llm_analyzed_functions) + len(self.llm_analyzed_macros) + len(self.llm_analyzed_views) + len(self.trigger_results)
        if total == 0:
            return "N/A"

        successful = sum(
            1 for result in (self.llm_analyzed_procedures + self.llm_analyzed_functions + self.llm_analyzed_macros + self.llm_analyzed_views + self.trigger_results)
    ...
    def _build_combined_summary(self, statement_report: Dict, procedure_output_files: Dict) -> Dict:
        ...
        total_triggers = len(self.trigger_results)
        successful_llm = sum(
            1 for r in (self.llm_analyzed_procedures + self.llm_analyzed_functions + self.llm_analyzed_macros + self.llm_analyzed_views + self.trigger_results)
            if r.get('analysis_success', False)
        )
        total_llm = total_procs + total_funcs + total_macros + total_views + total_triggers
        ...
        return {
            ...
            "overall_summary": {
                ...
                "volatile_tables": s.get('volatile_tables', 0),
            ...
            "llm_analysis_details": {
                ...
                "macros_analyzed": total_macros,
                "views_analyzed": total_views,
    ...
    def _build_legacy_procedure_report(self, llm_results: List[Dict]) -> Dict:
        ...
        for result in llm_results:
            ...
            elif 'FUNCTION' in stmt_type or 'UDF' in stmt_type:
                report['functions'][name] = entry
                summary['total_functions'] += 1
            elif 'MACRO' in stmt_type:
                report['macros'][name] = entry
                summary['total_macros'] += 1
            elif 'VIEW' in stmt_type:
                report['views'][name] = entry
        ...
        self._populate_called_by_relations(report['macros'], report['macros'])
        self._populate_called_by_relations(report['views'], report['views'])
        ...
        self._populate_called_by_relations(report['macros'], report['procedures'])
        self._populate_called_by_relations(report['procedures'], report['macros'])
        self._populate_called_by_relations(report['views'], report['procedures'])
        self._populate_called_by_relations(report['procedures'], report['views'])
    ...
    def _convert_llm_result_to_legacy_entry(self, result: Dict) -> Optional[Dict]:
        ...
        # Get the procedure/function/macro/view name to avoid treating it as a table
        proc_func_name = result.get('name', '')

        # Only add target to writes_tables if it's a write operation (not SELECT)
        # and it's not the procedure/function/macro/view name itself
        ...
        for obj in intermediate_objects or []:
            ...
            if 'VOLATILE' in obj_type:
                volatile_tables.add(tbl_name)
                write_tables.add(tbl_name)
            elif 'TEMP' in obj_type or tbl_name.startswith('#'):
            ...
            for column in obj.get('columns') or []:
                ...
                if ref:
                    written_columns.add(ref)
        entry['creates_volatile_tables'] = sorted(volatile_tables)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Teradata Database Lineage Analyzer with LLM Enhancement')
    ...
    parser.add_argument('--dialect', '-d', default='teradata', help='SQL dialect (default: teradata)')
    ...
    try:
        ...
        print(f"  Views: {results['statistics']['total_views']}")
        print(f"  Triggers: {results['statistics']['total_triggers']}")
