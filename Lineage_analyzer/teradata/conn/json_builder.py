"""
Enhanced Lineage JSON Report Builder for Teradata
Handles CTEs, volatile tables, macros, set operations, and complex dependencies
"""
...
@dataclass
class MacroInfo:
    """Teradata Macro definition information"""
    name: str
    defining_script: str
...
class EnhancedLineageJSONBuilder:
    """Build comprehensive lineage JSON report with Teradata-specific features"""
    ...
    def __init__(self, dialect: str, source_directory: str):
        ...
        self.macros: Dict[str, List[MacroInfo]] = defaultdict(list)
    ...
    def build_lineage_report(self, analysis_results: List[Dict[str, Any]]) -> Dict:
        ...
        # Step 2: Resolve CTEs and volatile table references
    ...
    def _process_file_result(self, result: Dict[str, Any]):
        ...
        for lineage in result.get('lineages', []):
            ...
            if hasattr(lineage, 'volatile_tables') and lineage.volatile_tables:
                for volatile_table in lineage.volatile_tables:
                    if volatile_table not in self.volatile_tables:
                        self.volatile_tables[volatile_table] = TableInfo(
                            table_name=volatile_table,
                            is_volatile=True
                        )
                        self.volatile_tables[volatile_table].definition_scripts.add(file_path)
                        self.stats['volatile_table_count'] += 1

            # Process macros (Teradata-specific)
            if hasattr(lineage, 'macro_definitions') and lineage.macro_definitions:
                for macro_name, macro_def in lineage.macro_definitions.items():
                    self.macros[macro_name].append(MacroInfo(
                        name=macro_name,
                        defining_script=file_path,
                        parameters=macro_def.get('parameters', []),
                        target_tables=macro_def.get('target_tables', []),
                        query_snippet=macro_def.get('query', '')[:500]
                    ))
    ...
    def _register_table(self, table_name: str, stmt_type: str, file_path: str, stmt_subtype: str = None):
        ...
        if table_name not in self.tables:
            self.tables[table_name] = TableInfo(
                ...
                is_volatile=self._is_volatile_table(table_name),
                ...
                is_macro=stmt_type == 'CREATE' and 'MACRO' in (stmt_subtype or '').upper()
            )
    ...
    def _build_json_report(self, execution_order: Dict) -> Dict:
        """Build final JSON report structure with Teradata-specific elements"""
        ...
        # Build volatile tables section (Teradata-specific)
        volatile_tables_json = {}
        for volatile_name, volatile_info in self.volatile_tables.items():
            volatile_tables_json[volatile_name] = {
                "columns": list(volatile_info.columns.keys()),
                "definition_scripts": list(volatile_info.definition_scripts)
            }

        # Build macros section (Teradata-specific)
        macros_json = {}
        for macro_name, macro_list in self.macros.items():
            macros_json[macro_name] = [{
                "defining_script": macro.defining_script,
                "parameters": macro.parameters,
                "target_tables": macro.target_tables,
                "query_snippet": macro.query_snippet
    ...
    def _is_volatile_table(self, table_name: str) -> bool:
        """Check if table is a Teradata volatile table"""
        volatile_prefixes = ['vt_', 'vol_', 'volatile_']
        return (any(table_name.lower().startswith(prefix) for prefix in volatile_prefixes) or
                table_name in self.volatile_tables)

    def _is_temp_table(self, table_name: str) -> bool:
        """Check if table is temporary"""
        temp_prefixes = ['temp_', 'tmp_', 'staging_', 'stg_', 'work_', 'wrk_']
...
if __name__ == "__main__":
    builder = EnhancedLineageJSONBuilder("teradata", "./sql_files")
