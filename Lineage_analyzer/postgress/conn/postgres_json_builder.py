...
class PostgreSQLLineageJSONBuilder:
    """Build comprehensive lineage JSON report with advanced features"""
        ...
        for lineage in result.get('lineages', []):
            ...
            if lineage.cte_definitions:
                for cte_name, cte_def in lineage.cte_definitions.items():
                    # Ensure query snippet is a string before slicing
                    raw_query = cte_def.get('query', '')
                    try:
...
if __name__ == "__main__":
    builder = PostgreSQLLineageJSONBuilder("postgres", "./sql_files")
