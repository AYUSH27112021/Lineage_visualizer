"""
Metadata Statement Builder for Snowflake
...
class MetadataStatementBuilder:
    ...
    def _process_result(self, result: Dict[str, Any], source_type: str):
        ...
        # Track Snowflake-specific features
        if result.get('has_variant_access'):
            self.stats['variant_access_count'] += 1

        if result.get('has_lateral_flatten'):
            self.stats['lateral_flatten_count'] += 1
...
if __name__ == "__main__":
    builder = MetadataStatementBuilder("snowflake", "TestDB")
    ...
    mock_view_results = [
        {
            ...
            'source_tables': ['PUBLIC.customers', 'PUBLIC.orders'],
            'column_lineage': [
                {
                    'target_column': 'customer_id',
                    'target_table': 'vw_customer_orders',
                    'source_columns': [{'table': 'PUBLIC.customers', 'column': 'customer_id'}],
