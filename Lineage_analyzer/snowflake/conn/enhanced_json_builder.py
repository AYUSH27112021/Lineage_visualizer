"""
Enhanced Procedure JSON Builder for Snowflake
...
class EnhancedProcedureJSONBuilder:
    ...
    def _build_summary_statistics(self) -> Dict[str, Any]:
        ...
        for result in self.analysis_results:
            ...
            if 'error' not in lineage:
                ...
                complexity_scores.append({
                    ...
                    'has_lateral_flatten': metrics.get('has_lateral_flatten', False),
                    'has_variant_access': metrics.get('has_variant_access', False)
                })
        ...
        complexity_scores.sort(
            key=lambda x: (
                ...
                (3 if x.get('has_lateral_flatten') else 0) +
                (2 if x.get('has_variant_access') else 0)
            ),
            reverse=True
        )
    ...
    def generate_summary_report(self) -> str:
        ...
        report = f"""
{'='*80}
SNOWFLAKE LINEAGE ANALYSIS SUMMARY REPORT (LLM-Based)
{'='*80}
...
"""
        ...
        for i, proc in enumerate(summary['most_complex_procedures'], 1):
            ...
            if proc.get('has_lateral_flatten'):
                report += f"      Has LATERAL FLATTEN: Yes\n"
            if proc.get('has_variant_access'):
                report += f"      Has Variant/JSON Access: Yes\n"
...
def main():
    ...
    print("Enhanced Executable Components JSON Builder for Snowflake (LLM-Optimized)")
    print("=" * 80)
    print("\nFeatures:")
    ...
    print("  - Snowflake-specific features (VARIANT, LATERAL FLATTEN)")
    print("\nUsage:")
