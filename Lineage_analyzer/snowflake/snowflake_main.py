"""
Main Orchestrator for Snowflake Lineage Analysis with LLM-based Procedure Analysis
...
For database connection-based analysis, use conn/metadata_lineage_main.py
"""
...
class LineageOrchestrator:
    ...
    def run_full_analysis(self, max_files: int = None) -> Dict:
        ...
        print("\n" + "="*80)
        print("SNOWFLAKE LINEAGE ANALYSIS PIPELINE (with LLM-Enhanced Procedure Analysis)")
        print("="*80)
        ...
        if self.task_results:
            print(f"   ✓ Found {len(self.task_results)} tasks")
        if self.stream_results:
            print(f"   ✓ Found {len(self.stream_results)} streams")

        # Step 3: Extract procedures, functions, tasks, and streams for LLM analysis
        print("\n[3/6] Extracting procedures, functions, tasks, and streams...")
        ...
        task_count = len([s for s in proc_func_statements if s['type'] == 'TASK'])
        if task_count > 0:
            print(f"   ✓ Found {task_count} tasks")

        # Step 4: Perform LLM-based analysis on procedures/functions/tasks
        print("\n[4/6] Analyzing procedures/functions/tasks with LLM...")
        ...
        if proc_func_statements:
            ...
            print("   ⚠ No procedures, functions, or tasks found to analyze")
        ...
        all_llm_results = self.llm_analyzed_procedures + self.llm_analyzed_functions + self.task_results
    ...
    def _analyze_statements(self):
        ...
        for file_path, content in self.cleaned_files.items():
            ...
            # Analyze views and include them in statement_results for column lineage
            if content.get('views'):
                for view in content['views']:
                    ...
                    # Add to statement_results so views are included in lineage report
                    self.statement_results.append(view_result)

                    # Also track separately for statistics
            ...
            # Track tasks (Snowflake-specific)
            ...
            # Track streams (Snowflake-specific)
            if content.get('streams'):
    ...
    def _extract_procedures_and_functions(self) -> List[Dict]:
        ...
        for file_path, content in self.cleaned_files.items():
            ...
            # Extract tasks (Snowflake-specific - can contain SQL logic)
            if content.get('tasks'):
                for task in content['tasks']:
                    ...
                    statement = {
                        'type': 'TASK',
                        'name': task_name,
                        'original_sql': task_content,
                        'modified_sql': task_content,
                        'file_path': str(file_path),
                        'schema_name': self._extract_schema_from_name(task_name),
                        'object_name': self._extract_object_name(task_name)
                    }
                    proc_func_statements.append(statement)
    ...
    def _extract_schema_from_name(self, full_name: str) -> str:
        """Extract schema from qualified name (e.g., 'PUBLIC.ProcName' -> 'PUBLIC')"""
        if '.' in full_name:
            return full_name.split('.')[0].strip('"')
        return 'PUBLIC'

    def _extract_object_name(self, full_name: str) -> str:
        """Extract object name from qualified name (e.g., 'PUBLIC.ProcName' -> 'ProcName')"""
        if '.' in full_name:
    ...
    def _calculate_llm_success_rate(self) -> str:
        ...
        total = len(self.llm_analyzed_procedures) + len(self.llm_analyzed_functions) + len(self.task_results)
        if total == 0:
            return "N/A"

        successful = sum(
            1 for result in (self.llm_analyzed_procedures + self.llm_analyzed_functions + self.task_results)
    ...
    def _build_combined_summary(self, statement_report: Dict, procedure_output_files: Dict) -> Dict:
        ...
        total_tasks = len(self.task_results)
        successful_llm = sum(
            1 for r in (self.llm_analyzed_procedures + self.llm_analyzed_functions + self.task_results)
            if r.get('analysis_success', False)
        )
        total_llm = total_procs + total_funcs + total_tasks
    ...
    def _convert_llm_result_to_legacy_entry(self, result: Dict) -> Optional[Dict]:
        ...
        # Get the procedure/function/task name to avoid treating it as a table
        proc_func_name = result.get('name', '')

        # Only add target to writes_tables if it's a write operation (not SELECT)
        # and it's not the procedure/function/task name itself
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Snowflake Database Lineage Analyzer with LLM Enhancement')
    ...
    parser.add_argument('--dialect', '-d', default='snowflake', help='SQL dialect (default: snowflake)')
    ...
    try:
        ...
        print(f"  Tasks: {results['statistics']['total_tasks']}")
