"""
Enhanced Lineage JSON Report Builder for Oracle
Handles CTEs, global temporary tables, PL/SQL blocks, and complex dependencies
"""
...
class EnhancedLineageJSONBuilder:
    """Build comprehensive lineage JSON report with Oracle-specific features"""
    ...
    def build_lineage_report(self, analysis_results: List[Dict[str, Any]]) -> Dict:
        ...
        # Step 2: Resolve CTEs and global temp table references
    ...
    def _process_file_result(self, result: Dict[str, Any]):
        ...
        for lineage in result.get('lineages', []):
            ...
            # Process global temporary tables (Oracle-specific)
            if hasattr(lineage, 'global_temp_tables') and lineage.global_temp_tables:
                for global_temp_table in lineage.global_temp_tables:
                    if global_temp_table not in self.global_temp_tables:
                        self.global_temp_tables[global_temp_table] = TableInfo(
                            table_name=global_temp_table,
                            is_global_temp=True
                        )
                        self.global_temp_tables[global_temp_table].definition_scripts.add(file_path)
    ...
    def _register_table(self, table_name: str, stmt_type: str, file_path: str, stmt_subtype: str = None):
        ...
        if table_name not in self.tables:
            self.tables[table_name] = TableInfo(
                ...
                is_global_temp=self._is_global_temp_table(table_name),
    ...
    def _build_json_report(self, execution_order: Dict) -> Dict:
        """Build final JSON report structure with Oracle-specific elements"""
        ...
        # Build global temporary tables section (Oracle-specific)
        global_temp_tables_json = {}
        for global_temp_name, global_temp_info in self.global_temp_tables.items():
            global_temp_tables_json[global_temp_name] = {
                "columns": list(global_temp_info.columns.keys()),
                "definition_scripts": list(global_temp_info.definition_scripts)
            }
        ...
        report = {
            ...
            "ctes": ctes_json,
            "global_temp_tables": global_temp_tables_json,
            "warnings": {
    ...
    def _is_global_temp_table(self, table_name: str) -> bool:
        """Check if table is an Oracle global temporary table"""
        global_temp_prefixes = ['gtt_', 'global_temp_', 'gt_']
        return (any(table_name.lower().startswith(prefix) for prefix in global_temp_prefixes) or
                table_name in self.global_temp_tables)
