"""
Enhanced Procedure Analyzer with LLM-based Lineage Extraction
Primary method: LLM analysis with table metadata context
Supports: Ollama and OpenAI with parallel async processing
Filters: Only procedures and functions (not views or query history)
"""
...
class EnhancedLLMLineageAnalyzer:
    ...
    def _build_table_metadata_index(self) -> Dict[str, Dict[str, Any]]:
        ...
        for table in tables:
            schema = table.get("schema") or table.get("schema_name") or "dbo"
            ...
            qualified_name_simple = f"{schema}.{name}"
            ...
            table_entry = {
                ...
                "qualified_name_simple": qualified_name_simple,
                "columns": columns,
                "type": "TABLE",
            ...
            index[qualified_name_simple.lower()] = table_entry
        ...
        for view in views:
            schema = view.get("schema_name") or "dbo"
            ...
            index[qualified_name_simple.lower()] = view_entry
    ...
    def _extract_table_references(self, sql: str) -> List[str]:
        ...
        # Pattern 1: [schema].[table] (SQL Server style)
        bracket_pattern = r'\[([^\]]+)\]\.\[([^\]]+)\]'
        for match in re.finditer(bracket_pattern, normalized_sql):
            schema, table = match.groups()
            ref = f"{schema}.{table}"
            referenced.add(ref)
        
        # Pattern 2: schema.table (without brackets)
        ...
        # Pattern 3: Standalone table names (check against metadata)
        for table_key, table_info in self.table_metadata_index.items():
            ...
            if re.search(pattern, normalized_sql):
                referenced.add(table_info['qualified_name_simple'])
    ...
    def _get_table_context(self, table_references: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for table_ref in table_references:
            ...
            if table_ref_lower in self.table_metadata_index:
                ...
                # Use qualified simple name as key
                key = table_info['qualified_name_simple']
                context[key] = {
    ...
    def _get_all_tables_list(self) -> str:
        ...
        for table_info in self.table_metadata_index.values():
            table_name = table_info['qualified_name_simple']
    ...
    def _create_enhanced_prompt(
        ...
        prompt = f"""Analyze this {statement_type} and extract column level data lineage. Return ONLY valid JSON.
...
{{
  ...
  "target": {{
    "name": "schema.table_or_view",
    "operation": "INSERT|UPDATE|DELETE|MERGE|CREATE_TABLE|CREATE_VIEW|CREATE_OR_REPLACE_VIEW",
    "object_type": "TABLE|VIEW|PROCEDURE|FUNCTION",
    "columns_affected": ["column1", "column2"]
  }},
  "additional_targets": [
    {{
      "name": "schema.other_target_written",
      "operation": "INSERT|UPDATE|CREATE_OR_REPLACE_VIEW|MERGE|DELETE",
      "object_type": "TABLE|VIEW|STREAM|LOG",
      "reason": "e.g. audit log write or secondary side-effect"
    }}
  ],
  "column_lineage": [
    {{
  ...
  "procedure_context": {{
    "container": "PROCEDURE_OR_PACKAGE_NAME_IF_APPLICABLE",
    "notes": "Describe the procedure purpose (e.g., refreshes view then logs audit)"
  }}
}}
...
5. List all called procedures/functions
...
7. Return ONLY JSON, no other text"""
            ...
            response = await asyncio.wait_for(
                self.openai_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a SQL lineage analysis expert. Return only valid JSON responses."
                        },
                        {
                            "role": "user",
                            "content": prompt
    ...
    def _is_procedure_or_function(self, statement: Dict[str, Any]) -> bool:
        """
        Filter to only process procedures, functions, and triggers.
        Excludes views, query history, and other statement types.
        """
        stmt_type = statement.get('type', '').upper()
        
        # Accept procedures, functions, and triggers
        accepted_types = ['PROCEDURE', 'FUNCTION', 'STORED_PROCEDURE', 'SCALAR_FUNCTION', 
                         'TABLE_FUNCTION', 'INLINE_FUNCTION', 'TRIGGER']
    ...
    async def _analyze_statement_async(
        ...
        # Check if this is a procedure or function
        ...
        elif stmt_type.upper() in ['FUNCTION', 'SCALAR_FUNCTION', 'TABLE_FUNCTION']:
            self.stats['functions'] += 1
        elif stmt_type.upper() == 'TRIGGER':
            self.stats['procedures'] += 1  # Count triggers with procedures for stats
    ...
    async def _process_batch(
        ...
        if self.use_openai:
            ...
            for i, result in enumerate(results):
                if result is None:
                    # Skipped (not a procedure/function)
                    continue
    ...
    async def analyze_statements_async(
        ...
        Only processes procedures and functions.
        ...
        Returns:
            List of analyzed procedures/functions with lineage information
        """
        # Filter to only procedures and functions
        ...
        print(f"\n{'='*80}")
        print(f"LLM-BASED LINEAGE ANALYSIS")
        print(f"{'='*80}")
...
def main():
    ...
    print("Enhanced LLM-Based Procedure Analyzer")
    print("=" * 80)
    print("\nFeatures:")
    ...
    print("  ✓ Only processes procedures & functions")
