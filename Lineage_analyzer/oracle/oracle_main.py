"""
Main Orchestrator for Oracle Lineage Analysis with LLM-based Procedure Analysis
...
class LineageOrchestrator:
    ...
    def __init__(
        ...
        self.cleaner = OracleSQLCleaner(sql_directory=sql_directory, debug=debug)
    ...
    def run_full_analysis(self, max_files: int = None) -> Dict:
        ...
        print("\n" + "="*80)
        print("Oracle LINEAGE ANALYSIS PIPELINE (with LLM-Enhanced Procedure Analysis)")
        print("="*80)
    ...
    def _analyze_statements(self):
        ...
        for file_path, content in self.cleaned_files.items():
            ...
            # Separately track views and ADD THEM TO STATEMENT RESULTS for column lineage
            if content.get('views'):
                for view in content['views']:
                    ...
                    # CRITICAL: Add view result to statement_results so it appears in final lineage report
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Oracle Database Lineage Analyzer with LLM Enhancement')
    ...
    parser.add_argument('--dialect', '-d', default='oracle', help='SQL dialect (default: oracle)')
