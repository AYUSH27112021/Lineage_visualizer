      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ width: '100%', maxWidth: '720px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 800 }}>Oracle Connection</h1>
              <p style={{ margin: '6px 0 0', color: '#9ca3af' }}>
                Provide Oracle connection details to extract metadata for lineage analysis.
              </p>
            </div>
            <button
              onClick={onBack}
          ...
          >
            ...
            <div>
              ...
              <input
                type="number"
                value={connectionDetails.port || '1521'}
            ...
            <div>
              <label style={labelStyle}>Service Name / SID</label>
              <input
                type="text"
                value={connectionDetails.serviceName || ''}
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={labelStyle}>Target Schemas (optional)</label>
              <input
                type="text"
                value={connectionDetails.targetSchemas || ''}
                onChange={(e) => updateField('targetSchemas', e.target.value)}
                placeholder="Comma separated schema list (defaults to user schema)"
                style={inputStyle}
              />
            </div>
          </div>
          ...
          <div style={{ padding: '20px', background: '#111827', border: '1px solid #374151', borderRadius: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              ...
              />
              <label htmlFor="oracleUseOpenAI" style={{ fontSize: '16px', fontWeight: 600, cursor: 'pointer' }}>
                Use OpenAI Key
              </label>
            </div>
